...
bool EstablishTCPConnection(
	struct sockaddr_in* clientAddr, // [in] адрес клиента
	SOCKET* tcpSocket               // [out] указатель на TCP сокет
);
...
int main(int aegc, _TCHAR* argv[]) {
	...
	try {
		...
		char serverName[] = "ConcurrentServer";
		short port = 2000;
		...
		while (true) {
			...
			if (GetRequestFromClient(serverName, port, (struct sockaddr*)&clientAddr, &clientAddrLen))
			{
				...
				SOCKET tcpSocket;
				if (EstablishTCPConnection(&clientAddr, &tcpSocket)) {
					cout << "TCP connection established with client!" << endl;

					/*const char* message = "Hello, TCP Client!";
					send(tcpSocket, message, strlen(message) + 1, 0);*/
...
bool EstablishTCPConnection(
	...
	clientAddr->sin_port = htons(3000); // TCP-порт клиента (должен быть известен)

	if (connect(sS, (sockaddr*)clientAddr, sizeof(*clientAddr)) == SOCKET_ERROR) {
		closesocket(sS);
