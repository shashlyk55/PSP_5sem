...
bool GetServerByName(
	...
	char* call,				// [in] позывной компьютера
...
int main(int argc, _TCHAR* argv[])
{
	...
	try {
		...
		if (GetServerByName(computerName, serverName, (sockaddr*)&servAddr, &servAddrLen)) {
			cout << "Server found " << endl;
			cout << inet_ntoa(servAddr.sin_addr) << ":" << htons(servAddr.sin_port) << endl;
		}
		else {
	...
	) 
{
	...
	struct hostent* he;
	if ((he = gethostbyname("DESKTOP-9MC22NJ")) == NULL) {
		throw SetErrorMsgText("gethostname: ", WSAHOST_NOT_FOUND);
	}
